$(document).ready(function(){
    ...
    if (0 == suitableGraphs.length) {
        ...
        if (1 < suitableGraphs.length && 0 < CubeViz_Parameters.chartType.length) {
            chartName = CubeViz_Parameters.chartType.charAt(0).toUpperCase() 
                        + CubeViz_Parameters.chartType.slice(1) + "Chart";
        } else {
        ...
        for ( var suitableGraph in suitableGraphs ) {
            ...
            $("#chart-selection").append (
                '<a href="?chartType=' + suitableGraph + '">' + 
                    '<img src="' + CubeViz_Parameters.cubevizImagesPath + suitableGraph + '.png">' + 
                '</a>'
            );
        }
    }
