...
class TableParser extends Parser {
    ...
    public function retrieveChartResult() {
        ...
        //create the headlines first
        $this->_htmlOutput = '<h2><center>'.$titles['title'].'</center></h2><br />';
        $this->_htmlOutput .= '<h3><center>'.$titles['subtitle'].'</center></h3><br />';
        
        $this->_htmlOutput .= '<table border="1" style="outline:1px;">';
        ...
        foreach($dimensionsReversed as $dimension) {
            ...
            $this->_th[$dimension] = '<th>'.$nameTable['d'][$dimension]['label'].
                    '</th>';
        ...
        $this->_htmlOutput .= '<th><center>'.$this->_translate->_('title measures').
                '</center></th>';
        $this->_htmlOutput .= '</tr>';
        ...
        if($index+1 < count($this->_dimensions)) {
            ...
            foreach($this->_dimensionElem[$dimension] as $elem => $state) {
                ...
                foreach($this->_dimensionData as $id => $tuple) {
                    ...
                    if(empty($comparison)) {
                        $this->_htmlOutput .= '<td rowspan="1"><center>';
                        foreach($this->_measureData[$id] as $measure => $value) {
                            $this->_htmlOutput .= $measure.':&nbsp;<b>'.$value.
                                    '</b><br />';
                        }
                        $this->_htmlOutput .= '</center></td>';
                        break;
                    }
                }
